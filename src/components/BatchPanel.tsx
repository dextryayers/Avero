...
export default function BatchPanel() {
  ...
  async function pickFiles() {
    try {
      const sel = await open({ multiple: true, filters: [{ name: "Image", extensions: ["png", "jpg", "jpeg", "webp", "psd"] }] });
      if (!sel) return;
      const arr = (Array.isArray(sel) ? sel : [sel]).map((p) => ({ path: p as string, name: (p as string).split(/[/\\]/).pop() ?? "file" }));
      enqueueBatch(arr);
    } catch {
  ...
  async function runBatch(presetId: string) {
    ...
    for (const item of useAutomationStore.getState().batch) {
      ...
      steps.forEach((st) => {
        if (st.action.type === "adjustment/exposure") useProStore.getState().addAdjustment("exposure");
        if (st.action.type === "adjustment/contrast") useProStore.getState().addAdjustment("brightnessContrast");
    ...
    <div className="space-y-3 p-3 text-[12px]">
      <section className="rounded border border-[#3e3e42] bg-[#2a2a2a] p-2">
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <span className={`rounded-full px-2 py-0.5 text-[10px] ${recording ? "bg-red-600 text-white" : "bg-[#3e3e42]"}`}>{recording ? "REC" : "IDLE"}</span>
        </div>
        {!recording ? (
          <button onClick={startRec} className="w-full rounded bg-[#0a84ff] px-2 py-1.5 text-white hover:bg-[#0070e0]">Mulai rekam</button>
        ) : (
          <div className="grid grid-cols-2 gap-1">
            <button onClick={stopRec} className="rounded bg-[#2d7a3a] px-2 py-1.5 text-white">Stop</button>
            <button onClick={clearMacro} className="rounded bg-[#3e3e42] px-2 py-1.5">Clear</button>
          </div>
        )}
        <div className="mt-1.5 space-y-1">
          {macro.length === 0 && <div className="text-[11px] text-[#a0a0a0]">Belum ada langkah. Nyalakan REC lalu tambah adjustment/filter.</div>}
          {macro.map((m) => (
            <div key={m.id} className="rounded bg-[#1e1e1e] px-2 py-1 font-mono text-[10px] text-[#c5c5c5]">{m.label}</div>
          ))}
        </div>
        {macro.length > 0 && (
          <button
        ...
        <div className="space-y-1.5">
          {presets.map((p) => (
            <div key={p.id} className="rounded border border-[#3e3e42] bg-[#2a2a2a] p-2">
              ...
              <button onClick={() => runBatch(p.id)} className="mt-1 w-full rounded bg-[#3e3e42] px-2 py-1 text-[11px] hover:bg-[#505050]">
      ...
      <section>
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <button onClick={clearBatch} className="rounded bg-[#3e3e42] px-2 py-1 text-[10px]">Clear</button>
        </div>
        <button onClick={pickFiles} className="w-full rounded bg-[#252526] border border-dashed border-[#505050] px-2 py-2 text-[11px] hover:bg-[#2d2d2d]">
        ...
        <div className="mt-1.5 max-h-44 space-y-1 overflow-y-auto">
          ...
          {batch.map((b) => (
            <div key={b.id} className="rounded bg-[#1e1e1e] px-2 py-1.5 text-[11px]">
              <div className="flex justify-between"><span className="truncate text-white">{b.name}</span><span className="font-mono">{b.status}</span></div>
