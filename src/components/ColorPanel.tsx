        ...
        <div className="grid grid-cols-3 gap-1">
          {(["sRGB", "AdobeRGB", "ProPhoto"] as const).map((w) => (
            <button
              ...
              className={`rounded px-2 py-1.5 text-[11px] ${color.workingSpace === w ? "bg-[#0a84ff] text-white" : "bg-[#3e3e42] hover:bg-[#505050]"}`}
            >
              {w}
            </button>
          ))}
        ...
        <div className="mt-1.5 flex gap-1">
          {([8, 16] as const).map((b) => (
            <button
              ...
              className={`flex-1 rounded px-2 py-1 text-[11px] ${color.bitDepth === b ? "bg-[#2d7a3a] text-white" : "bg-[#3e3e42]"}`}
            >
              {b}-bit {b === 16 ? "(sim)" : ""}
            </button>
          ))}
