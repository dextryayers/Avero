  ...
  return (
    <div className="space-y-3 p-3 text-[12px]">
      <div className="rounded border border-[#3e3e42] bg-[#2d2d2d] p-2 text-[11px] text-[#a0a0a0]">
        ...
        <br />
        Mask putih tampil, hitam sembunyi. Paint dengan Brush putih dan Eraser hitam saat mode paint mask aktif.
      </div>
      {!m?.hasMask ? (
        <button
          onClick={() => {
        ...
        <>
          <label className="flex items-center justify-between text-[#c5c5c5]">
            ...
            <input type="checkbox" checked={m.enabled} onChange={(e) => activeLayerId && updateMask(activeLayerId, { enabled: e.target.checked })} />
          </label>
          <label className="flex items-center justify-between text-[#c5c5c5]">
            <span className={paintMask ? "text-white font-semibold" : ""}>Paint mask mode</span>
            <input type="checkbox" checked={paintMask} onChange={(e) => setPaintMask(e.target.checked)} />
          </label>
          <div>
            <label className="mb-1 flex justify-between text-[#a0a0a0]">
              Feather <span className="font-mono text-white">{m.feather}px</span>
            </label>
            <input type="range" min={0} max={24} value={m.feather} onChange={(e) => activeLayerId && updateMask(activeLayerId, { feather: Number(e.target.value) })} className="w-full" />
          </div>
          <div>
            <label className="mb-1 flex justify-between text-[#a0a0a0]">
              Density <span className="font-mono text-white">{m.density}%</span>
            </label>
            <input type="range" min={0} max={100} value={m.density} onChange={(e) => activeLayerId && updateMask(activeLayerId, { density: Number(e.target.value) })} className="w-full" />
          </div>
          <div>
            <label className="flex items-center justify-between text-[#c5c5c5]">
              <span>Clipping ke layer bawah</span>
              <input type="checkbox" checked={!!active?.clipped} onChange={(e) => activeLayerId && updateLayer(activeLayerId, { clipped: e.target.checked })} />
            </label>
            <p className="mt-1 text-[10px] text-[#a0a0a0]">Clipping tahap 1: flag tersimpan, full isolate masuk Fase 5 node graph.</p>
          </div>
          <button
            onClick={() => {
              if (!activeLayerId) return;
