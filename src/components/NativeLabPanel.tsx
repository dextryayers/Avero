    ...
    <div className="space-y-3 p-2 text-[12px]">
      <div className="avero-card overflow-hidden p-0">
        <div className="flex items-center gap-2 border-b border-[#2c2c31] bg-[#161618] px-3 py-2.5">
          ...
          <div>
            <div className="text-[11.5px] font-bold text-white">Pusat Memori</div>
            <div className="font-mono text-[10px] text-[#6e6e78]">Anggaran RAM dokumen</div>
          </div>
          <span className={`ml-auto rounded px-1.5 py-0.5 font-mono text-[10px] ${nat?.ready ? "bg-[#232327] text-[#8fb6f5]" : "bg-[#2c2c31] text-[#6e6e78]"}`}>{nat?.ready ? "siap" : "web"}</span>
        </div>
          ...
          <div className="col-span-2 rounded-md border border-[#2c2c31] bg-[#161618] p-2">
            ...
            {budget && (
              <div className="mt-1.5 grid grid-cols-2 gap-1 font-mono text-[10px] text-[#6e6e78]">
                <span>Penuh {budget.total_mb.toFixed(1)} MB</span>
                <span>Hemat {budget.light_total_mb.toFixed(1)} MB</span>
                <span className="col-span-2 text-[#8fb6f5]">Hemat {budget.light_saving_mb.toFixed(1)} MB dengan mode ubin</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="avero-card p-3">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-4 text-[11px] leading-relaxed text-[#a7a7b0]">
          <li><span className="font-semibold text-white">Diproses di tempat</span> - tanpa salinan baru, hanya tabel 768B atau histogram 1KB.</li>
          <li><span className="font-semibold text-white">Diproses per ubin</span> - ubin 512, tambahan &lt;64KB dibanding salinan penuh {budget ? formatMB(budget.bytes_per_layer/1024/1024) : "-"}.</li>
          <li><span className="font-semibold text-white">Antrean bertumpuk</span> - hanya 2 penyangga yang dipakai bergantian, dipakai ulang.</li>
          <li><span className="font-semibold text-white">Responsif</span> - kanvas dipecah per ubin agar antarmuka tetap mulus.</li>
        </ul>
      </div>
      ...
      <div className="rounded-md border border-dashed border-[#2c2c31] p-3 text-center text-[11px] text-[#6e6e78]">
        Tips: untuk foto 45MP (8192×5464), mode penuh ~537MB, mode hemat ~273MB, hemat ~264MB.
      </div>
    </div>
  );
}
