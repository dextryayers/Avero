  ...
  if (!enabled) {
    return (
      <div className="border-b border-[#3e3e42] bg-[#191d24] px-3 py-1.5 text-[11px] text-[#a0a0a0]">
        ...
        <button onClick={() => { toggle(); autoFromStack(); }} className="ml-2 rounded bg-[#7a4fd0] px-2 py-0.5 text-white">Aktifkan node view</button>
      </div>
    );
  }
  ...
  return (
    <div className="border-b border-[#7a4fd0] bg-[#14161d]">
      <div className="flex items-center gap-1.5 px-3 py-1.5 text-[11px]">
        ...
        <span className="text-[#a0a0a0]">{nodes.length} nodes • {edges.length} links • auto-convert dari stack</span>
        <button onClick={autoFromStack} className="ml-1 rounded bg-[#3e3e42] px-2 py-0.5 hover:bg-[#505050]">Rebuild</button>
        <button onClick={() => addNode("adjust", "Adjust")} className="rounded bg-[#0a84ff] px-2 py-0.5 text-white">+Adjust</button>
        <button onClick={() => addNode("filter", "Filter")} className="rounded bg-[#7a4fd0] px-2 py-0.5 text-white">+Filter</button>
        {selected && <button onClick={() => removeNode(selected)} className="rounded bg-[#5a1f1f] px-2 py-0.5 text-white">Hapus node</button>}
        <button onClick={toggle} className="ml-auto rounded bg-[#2a2a2a] px-2 py-0.5">Tutup</button>
      </div>
      <div
      ...
      >
        <svg className="absolute inset-0 h-full w-full">
          {edges.map((e) => {
            ...
            if (!a || !b) return null;
            return <line key={e.id} x1={a.x + 96} y1={a.y + 24} x2={b.x} y2={b.y + 24} stroke="#7a8aa0" strokeWidth={1.6} />;
          })}
        </svg>
        {nodes.map((n) => (
          <div
      ...
      <div className="px-3 pb-1.5 text-[10px] text-[#7a8aa0]">Klik node untuk pilih, double-klik node lain untuk sambung. Toggle di node mematikan adjustment/filter asli secara live.</div>
    </div>
  );
}
...
function ToggleAdjust({ refId }: { refId: string }) {
  ...
  if (!adj) return null;
  return (
    <button onClick={(e) => { e.stopPropagation(); update(refId, { enabled: !adj.enabled }); }} className="mt-1 w-full rounded bg-black/40 px-1 py-0.5 text-white">
      {adj.enabled ? "ON" : "OFF"}
    </button>
  );
}
...
function ToggleFilter({ refId }: { refId: string }) {
  ...
  if (!f) return null;
  return (
    <button onClick={(e) => { e.stopPropagation(); update(refId, { enabled: !f.enabled }); }} className="mt-1 w-full rounded bg-black/40 px-1 py-0.5 text-white">
      {f.enabled ? "ON" : "OFF"}
    </button>
  );
}
