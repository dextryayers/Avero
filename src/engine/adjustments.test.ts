...
function img2x2(): ImageData {
  const d = new Uint8ClampedArray([100, 100, 100, 255, 200, 200, 200, 255, 50, 50, 50, 255, 255, 255, 255, 255]);
...
describe("adjustments", () => {
  it("invert membalik channel", () => {
    const img = img2x2();
    applyAdjustmentToImageData(img, { id: "t", type: "invert", name: "Invert", enabled: true, opacity: 100, params: {} });
    expect(img.data[0]).toBe(155);
    expect(img.data[4]).toBe(55);
  });
  it("threshold menghasilkan biner", () => {
    const img = img2x2();
    applyAdjustmentToImageData(img, { id: "t", type: "threshold", name: "T", enabled: true, opacity: 100, params: { level: 128 } });
